              ...
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont"
                      :class="{ 'icon-arrowup': isAsc, 'icon-arrowdown': isDesc }"></span></a>
                </li>
...
export default {
  ...
  // 在挂载之前调用一次|可以在发送请求之前将带有参数进行修改
  // 在发送请求之前，把接口需要传递的参数进行整理（在服务器发送请求之前，把参数整理好，服务器就会返回查询的数据）
  beforeMount() {
    // 复杂的写法
  ...
  // 组件挂载完毕执行一次[仅仅执行一次]
  mounted() {
    // 在发送请求之前带给服务器参数[searchParams参数发生变化的时候,将数值带给服务器]
    this.getData()
  },
  computed: {
    // 注意:mapGetters里面的写法:传递的是数组,因为getters是没有划分模块的
  ...
  methods: {
    ...
    changeOrder(flag) {
      ...
      let newOrder = ''
      // 判断点击的是不是和上一次状态或者初始状态相同的flag
      // 如果是，则每次只需要将点击的flag传入，排序的方式取反即可
      if (flag == orignFlag) {
        newOrder = `${flag}:${orignSort == 'desc' ? 'asc' : 'desc'}`
      } else {
        // 如果点击的不是同一个falg，则将flag传入，默认排序方式为降序
        // 点击的是价格  flag=2
        newOrder = `${flag}:${'desc'}`
      }
      // 将新的排序方式赋值给searchParams
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      // 每一次请求发送之前，应该把上一次请求相应的1、2、3级分类id置空，以便接收本次的1、2、3级id
      // 分类的名字和关键字不用清理，因为每一次路由发生变化的时候，都会被赋予新的值
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
      // 再次发送请求之前，需要再次整理带给服务器的参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params)
      // 再次发起ajax请求
      this.getData()
