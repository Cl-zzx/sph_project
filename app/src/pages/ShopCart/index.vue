<template>
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a @click="dleteAllCheckedCart">删除选中的商品</a>
...
export default {
  ...
  methods: {
    ...
    /* async handler(type, disNum, cart) {
      // type:为了区分这三个元素
      // disNum：变化量（+1） 变化量（-1） input最终的个数（不是变化量）
      // cart：哪一个产品（身上有skuid可以标识）
      // 向服务器发送请求，修改数量
      switch (type) {
        // 加号
        case 'add':
          ...
          break;
        case 'minus':
          // 如果产品的个数大于1，才可以传给服务器-1，如果产品个数等于1，应该传递0
          disNum = cart.skuNum > 1 ? -1 : 0
          break
        case 'change':
          // 如果用户输入的最终量是非法的或者出现负数，那么带给服务器的改变量是0
  ...
  computed: {
    ...
    // 判断底部复选框是否勾选【全部产品勾选，才勾选】
    isAllChecked() {
      // every函数
      // 遍历数组里面的元素，只要全部元素的isChecked属性都为1---返回true
      // 遍历数组里面的元素，只要有一个元素的isChecked属性都不为1---返回false
