        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前进行路由跳转：从A路由跳转到B路由，这里加入购物车，进行路由跳转之前，发请求把你购买的产品信息通过请求的方式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    // 产品的售卖属性高亮值切换
    changeActive(spuSaleAttrValue, spuSaleAttrValueArr) {
      // 遍历全部售卖属性值，isChecked为0没有高亮
      spuSaleAttrValueArr.forEach(item => {
        item.isChecked = 0
      });
      // 点击的那个售卖属性设置为高亮
    ...
    changeSkuCount(event) {
      ...
      if (isNaN(value) || value < 1) {
        ...
        // 正常大于1【但是要求是整数,不能出现小数】
        this.skuCount = Math.floor(value)
      }

    },
    // 加入购物车的回调函数
    async addShopCart() {
      // 1.发起请求---将产品添加到数据库（通知服务器）
      /*
        当前这里是派发一个action,也向服务器发送请求，判断加入购物车是否成功，从而进行相应的操作
         this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum :this.skuCount})
         上面这行代码说白了：调用仓库的addOrUpdateShopCart方法，这个方法使用async修饰，返回的一定是一个Promise
         要么成功，要么失败
       */
      // 2.服务器存储成功---进行路由跳转，并传递参数
      // 3.失败--给用户进行提示
      try {
        await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuCount })
        // 进行路由跳转
        // 在路由跳转的时候还需要将产品的信息带给下一级的路由组件
        // 下面这种手段路由跳转以及传递参数是可以的
        // 一些简单的数据比如skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】，通过会话存储（不持久化，会话结束数据再消失）
        // 本地存储|会话存储，一般存储的是字符串
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        this.$router.push({ name: 'addcartsuccess', query: { skuNum: this.skuCount } })
      } catch (error) {
        alert(error.message)
      }
    }
